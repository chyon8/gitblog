          ...
          <Box sx={{color: '#FFFFFF',borderRadius:'15px', display: 'grid',
            ...
            {repos.map((repo) => (
              <Box sx={{cursor:'pointer'}} key={repo.id} onClick={() => fetchCommits(repo.name)}>
               ...
               <Link style={{textDecoration:'none'}} href={repo.html_url}> <Typography sx={{fontSize:'12px', color:'#CCCCCC' ,cursor: 'pointer',whiteSpace:'nowrap',overflow:'hidden' }}>Go to github</Typography></Link>
                </Box>
                </Box>
              </Box>
            ))}
